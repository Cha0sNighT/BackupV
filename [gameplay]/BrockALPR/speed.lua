		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						radar.info = string.format("~y~Plate: ~w~%s  ~y~Model: ~w~%s  ~y~Speed: ~w~%s kph", fplate, fmodel, math.ceil(fvspeed))
					end
					...
					if IsEntityAVehicle(j) then
						...
						radar.info2 = string.format("~y~Plate: ~w~%s  ~y~Model: ~w~%s  ~y~Speed: ~w~%s kph", bplate, bmodel, math.ceil(bvspeed))
