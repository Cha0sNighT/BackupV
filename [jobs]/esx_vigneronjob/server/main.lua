...
AddEventHandler('esx_vigneronjob:startHarvest', function(zone)
	...
	if PlayersHarvesting[_source] == false then
		TriggerClientEvent('esx:showNotification', _source, '~r~Ressortez de la zone~w~')
...
local function Sell(source, zone)
	...
	if PlayersSelling[source] == true then
		...
		if zone == 'SellFarm' then
			...
			if wine == 0 and jus == 0 and grand_cru == 0 then
				TriggerClientEvent('esx:showNotification', source, _U('no_product_sale'))
				return
			elseif xPlayer.getInventoryItem('wine').count <= 0 and jus == 0 and grand_cru == 0 then
				TriggerClientEvent('esx:showNotification', source, _U('no_vin_sale'))
				wine = 0
				return
			elseif xPlayer.getInventoryItem('jus_raisin').count <= 0 and wine == 0 and grand_cru == 0 then
				TriggerClientEvent('esx:showNotification', source, _U('no_jus_sale'))
				jus = 0
				return
			elseif xPlayer.getInventoryItem('grand_cru').count <= 0 and wine == 0 and jus == 0 then
				TriggerClientEvent('esx:showNotification', source, _U('no_jus_sale'))
				grand_cru = 0
				return
			else
				...
				elseif (grand_cru == 1) then
					SetTimeout(1100, function()
						local money = math.random(250,300)
