...
function SelectFuneral()
    local index = GetRandomIntInRange(1,  #Config.Funeral)

    for k,v in pairs(Config.Zones) do
      if v.Pos.x == Config.Funeral[index].x and v.Pos.y == Config.Funeral[index].y and v.Pos.z == Config.Funeral[index].z then
        return k
      end
    end
end
...
function StartNPCJob()
    NPCTargetFuneral     = SelectFuneral()
    local zone            = Config.Zones[NPCTargetFuneral]

    Blips['NPCTargetFuneral'] = AddBlipForCoord(zone.Pos.x,  zone.Pos.y,  zone.Pos.z)
    SetBlipRoute(Blips['NPCTargetFuneral'], true)
...
function StopNPCJob(cancel)
    ...
    if Blips['NPCTargetFuneral'] ~= nil then
      RemoveBlip(Blips['NPCTargetFuneral'])
      Blips['NPCTargetFuneral'] = nil
	end
    ...
    if cancel then
	  ESX.ShowNotification(_U('cancel_mission'))
		...
		TriggerServerEvent('esx_funeral:GiveItem')
		StartNPCJob()
		Done = true
	end
end
...
AddEventHandler('esx_funeral:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_funeral:hasExitedMarker', function(zone)

	if zone == 'Vente' then
		TriggerServerEvent('esx_funeral:stopVente')
	end
    CurrentAction = nil
	ESX.UI.Menu.CloseAll()
end)
	...
	while true do
		...
		if PlayerData.job ~= nil then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_funeral:hasEnteredMarker', currentZone)
			end
			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_funeral:hasExitedMarker', LastZone)
			end
		end
	end
end)
            ...
            if (IsControlJustReleased(1, Keys["E"]) or IsControlJustReleased(2, Keys["RIGHT"])) and PlayerData.job ~= nil then
				...
				if PlayerData.job.name == Config.nameJob then
					...
					if CurrentAction == 'vente' then
						TriggerServerEvent('esx_funeral:startVente')
					end
	...
	while true do
		...
		if IsControlJustReleased(1, Keys["DELETE"]) and PlayerData.job ~= nil and PlayerData.job.name == Config.nameJob then
			...
			if Onjob then
				...
				if IsPedInAnyVehicle(playerPed,  false) and IsVehicleModel(GetVehiclePedIsIn(playerPed,  false), GetHashKey("lurcher")) then
					StartNPCJob()
					Onjob = true
				else
